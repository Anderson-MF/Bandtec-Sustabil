...
db.conectar()
    .then(() => {
        const sql = `
        INSERT into dbo.leitura (umidade, momento, fksensor)
        values (${Humidity+20}, '${agora()}', 1);
        INSERT into dbo.leitura (umidade, momento, fksensor)
        values (${Humidity+20}, '${agora()}', 2);
        INSERT into dbo.leitura (umidade, momento, fksensor)
        values (${Humidity-20}, '${agora()}', 3);
        INSERT into dbo.leitura (umidade, momento, fksensor)
        values (${Humidity-20}, '${agora()}', 4);`;
        console.log(sql);
